...
DROP DATABASE IF EXISTS `sast_extdoor`;
CREATE DATABASE `sast_extdoor` /*!40100 DEFAULT CHARACTER SET utf8 */;
USE `sast_extdoor`;

DROP TABLE IF EXISTS `sast_acl_table`;
CREATE TABLE `sast_acl_table` (
  `openid` varchar(32) NOT NULL COMMENT '微信openid',
  `stu_id` int(11) NOT NULL COMMENT '学号',
  `permit` tinyint(4) NOT NULL DEFAULT '1' COMMENT '是否允许开门',
  UNIQUE KEY `openid` (`openid`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='权限控制表';


DROP TABLE IF EXISTS `sast_area_code`;
CREATE TABLE `sast_area_code` (
  `code` int(11) NOT NULL COMMENT '代码',
  `area` text NOT NULL COMMENT '地区',
  PRIMARY KEY (`code`),
  KEY `code` (`code`),
  FULLTEXT KEY `area` (`area`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='身份证前6位地区对应码';

INSERT INTO `sast_area_code` (`code`, `area`) VALUES
(110000,	'北京市'),
(110100,	'北京市市辖区'),
(110101,	'北京市东城区'),
(110102,	'北京市西城区'),
...
DROP TABLE IF EXISTS `sast_door_log`;
CREATE TABLE `sast_door_log` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `time` int(11) NOT NULL COMMENT '开门时间',
  `stu_id` int(11) NOT NULL COMMENT '开门者',
  `result` tinyint(4) NOT NULL COMMENT '开门结果',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='开门日志记录';


DROP TABLE IF EXISTS `sast_student_info`;
CREATE TABLE `sast_student_info` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  ...
  `hometown` text COMMENT '故乡',
  `gender` text COMMENT '性别 未知，男女',
  `department` text COMMENT '学系',
  PRIMARY KEY (`id`),
